...
if(isset($_GET["validar"]) && $_GET["validar"] == "form" ){
	...
	if( $txtImagem == $valorImagem ){
		...
		?> <script language="javascript"> alert ('<? echo "Desculpe ". $_POST["Nome"] .", os números digitados estão incorretos!"; ?>') </script> <?
	}
...
<form id="form1" name="frmImgValida" method="post" action="form.php?validar=form">
  <table width="30%" border="0" cellspacing="2">
    ...
    <tr>
      <td><span class="Estilo4">
        <label>
        <input name="txtImagem" type="text" id="txtImagem" size="20" placeholder="Quais números vê ao lado?" />
        </label>
      </span>
